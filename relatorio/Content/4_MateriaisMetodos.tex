...
\hspace{0,5cm}
  ...
  \subsection{IPCam} % rebs
    ...
    O \emph{script videostream.cgi} realiza a captura e envio do video através da rede para o cliente, enquanto que o \emph{decoder\_control.cgi} controla a movimentação da câmera em sua base. Abaixo (tabela \ref{comandos_ipcam}) \cite{ipcam} se destaca os comandos de movimentação que foram utilizados no projeto.
  ...
  \subsection{Flask}
    ...
    \emph{Jinja2} e boas intenções''. Com o \emph{Flask} podemos facilmente
    criar um \emph{backend} para processar as requisições à Intel Galileo,
...
\hspace{0,5cm}
  ...
  Nesta seção serão apresentados todos os métodos, implementações e comportamentos dos componentes desenvolvidos para se construir o sistema R3V. Com isso, irá se discursar um pouco sobre \emph{streaming} de video (MJPEG), mapeamento de movimento (\emph{dead reckoning}) e o servidor desenvolvido.
  ...
  \subsection{\emph{Dead Reckoning}} % rebs
    ...
    Utilizando os conceitos de \emph{dead reckoning} (posicionamento de acordo com leitura de sensores e estimativa de movimento), uma transformação matemática foi aplicada aos resultados do sensor inercial (acelerômetro) adiquiridos através do aplicativo do \emph{smartphone}, resultando em um discretização do espaço de visão do usuário. Isso se deve ao fato de que a IPCam possui somente movimentos bem definidos e não responde a movimentos fora de seus comandos pré-definidos.

    Através do SDK do \emph{Cardboard} da Google\textsuperscript{\textregistered} foi possível adiquirir os movimentos angulares denominados \emph{pitch} e \emph{yaw}, que correspondem a movimentos em torno de um eixo bem definido (eixo $x$ e $y$ respectivamente).
  ...
  \subsection{Aplicação Cliente} %davi
